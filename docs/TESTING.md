...
account-gated API (Discord, Slack, Mattermost webhooks; Telegram, Pushover, Matrix
bot accounts) or exist purely to talk to a specific external product (a generic
webhook receiver; Gatus). This harness has no real account or instance for any of
...
- **telegram** and **pushover** built their target URL from a hardcoded constant
  ...
  adding optional settings — `api_base` for telegram, `endpoint` for pushover —
  that override the default when set and change nothing for existing
  configuration when left unset. They exist for testability, not for routing
...
creates is named with the `beacon-itest-` prefix; nothing else is touched. Cleanup
runs on exit — success, failure, or interrupt — so a failed run doesn't leave
containers or the network behind.

Flags:

- `--keep` — skip cleanup at the end, to poke at a failure by hand
